...
const getGameMeta = async (dbUrl) => {
    return await gamesModel.findOne({dbUrl: dbUrl}).lean().exec()
}

const increaseClick = async (dbUrl) => {
    return await gamesModel.findOneAndUpdate({dbUrl: dbUrl}, {$inc: {clicks: 1}}, {new: true}).lean().exec()
}
